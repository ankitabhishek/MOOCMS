...
$page_title = "Manage Students | MOOCMS Admin";
...
if($_SERVER['REQUEST_METHOD']=='POST')
{
	...
	if(empty($errors))
	{
		...
		$q = "SELECT student_id, first_name, last_name, email FROM student WHERE first_name = '".mysql_real_escape_string($first_name)."' OR last_name = '".mysql_real_escape_string($last_name)."' OR email = '".mysql_real_escape_string($email)."' LIMIT 0,20";
		...
		if(mysqli_num_rows($r) >= 1)
		{
			...
			while($row = mysqli_fetch_array($r, MYSQLI_ASSOC))
			{
				...
				echo "<tr>
						<td><a href='edit_student.php?id={$row['student_id']}'>{$row['student_id']}</a></td>
	...
	//SEARCH FORM
	echo '<form action="manage_students.php" method="POST">
	...
	//count no. of ITEMS
	$q = "SELECT count(student_id) FROM student";
	$r = mysqli_query($dbc, $q);
	...
	$q = "SELECT student_id, first_name, last_name, email FROM student ORDER BY first_name ASC LIMIT $offset, $rec_limit";
	$r = mysqli_query($dbc, $q);
	...
	if($page == 0)
	{
		if($left_rec < $rec_limit)
		{
			...
			echo "<a href=\"manage_students.php?page=$page\">Next 10 Records</a>";
	...
	{
		$last = $page - 2;
		echo "<a href=\"manage_students.php?page=$last\">Last 10 Records</a>";
