...
if($_SERVER['REQUEST_METHOD']=='POST')
{
	...
	if(empty($errors))
	{
		...
		$q = "SELECT c.course_id, c.course_name, c.start_date FROM course c, enrollment e WHERE c.course_id = e.course_id AND e.student_id = '$student_id' AND c.course_name LIKE '%".mysql_real_escape_string($course_name)."%' LIMIT 0,20";
	...
	//SEARCH FORM
	echo '<form action="my_courses.php" method="POST">
	...
	//count no. of ITEMS
	$q = "SELECT count(course_id) FROM enrollment WHERE student_id = '$student_id'";
	$r = mysqli_query($dbc, $q);
	...
	$q = "SELECT c.course_id, c.course_name, c.start_date FROM course c, enrollment e WHERE c.course_id = e.course_id AND e.student_id = '$student_id' ORDER BY course_name ASC LIMIT $offset, $rec_limit";
	$r = mysqli_query($dbc, $q);
